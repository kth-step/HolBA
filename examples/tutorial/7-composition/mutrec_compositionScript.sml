...
(* The goal is to use the contract from mutrec_wpScript to prove the two contracts
   which is achieved by using the rules PreStr, IL-to-EL, Loop and PostWeak whose
   applications are automated by the functions
   ...
   respectively.
   In addition, to achieve this, we use the implications from mutrec_smtScript
   to resolve side conditions of the rules:

     contract_ev_1_imp_taut_thm
     contract_ev_2_imp_taut_thm
     contract_ev_3_imp_taut_thm
     contract_ev_4_imp_taut_thm
     contract_od_1_imp_taut_thm
     contract_od_2_imp_taut_thm
     contract_od_3_imp_taut_thm
     contract_od_4_imp_taut_thm


*)

(* =============================================================== *)
(* Obtain contracts using bir_simp_jgmt (usable with logic) from contracts
 * generated by the WP which use bir_exec_to_labels_triple. *)
...
(* is_even *)
  ...
  (* Prove theorem stating equivalence of different formats of postconditions:
   * we need to have it in a leading lambda-label format *)
  ...
  (* Rewrite both contracts using the above *)
  val loop_simp_ct_ = REWRITE_RULE [Once abs_ev_intro] bir_ieo_sec_iseven_loop_comp_ct;
  val ht = REWRITE_RULE [Once abs_ev_intro2] bir_ieo_sec_iseven_exit_comp_ct;

  (* Drop the loop point 0 from the include list of loop exit contract *)
  ...
  (* Weaken the postcondtion of the loop exit contract to also include the invariant
   * holding upon encountering the loop point 0 *)
  val loop_exit_simp2_ht =
    use_post_weak_rule_simp loop_exit_simp_ht ``BL_Address (Imm32 0x000w)`` contract_ev_3_imp_taut_thm;

  (* Expand some definitions *)
  ...
  val loop_ev_exit_ht = REWRITE_RULE [bir_ieo_sec_iseven_exit_pre_def,
                                   bir_ieo_variant_def] loop_exit_simp2_ht;

  (* Move some labels from the include list to the exclude list *)
  val loop_ev_simp_ct    = REWRITE_RULE [GSYM bir_ieo_sec_iseven_loop_post_def, Once abs_ev_intro]
          (bir_populate_elist_predset
...
(* is_odd *)
  ...
  val ht = REWRITE_RULE [Once abs_od_intro2] bir_ieo_sec_isodd_exit_comp_ct;

  val new_ilist = ``{BL_Address (Imm32 0x204w); BL_Address (Imm32 0x200w)}``;
  val loop_exit_simp_ht = bir_remove_labels_from_ilist_predset ht new_ilist;

  val loop_exit_simp2_ht =
    use_post_weak_rule_simp loop_exit_simp_ht ``BL_Address (Imm32 0x000w)`` contract_od_3_imp_taut_thm;
  ...
  val loop_od_simp_ct    = REWRITE_RULE [GSYM bir_ieo_sec_isodd_loop_post_def, Once abs_od_intro]
          (bir_populate_elist_predset
...
(* For debugging:
  val loop_map_ct   = loop_od_simp_ct;
...
val loop_and_exit_ev_ht =
  bir_compose_simp_loop_unsigned_predset
    loop_ev_simp_ct loop_ev_exit_ht ``bir_ieo_invariant v1`` ``bir_ieo_condition`` ``bir_ieo_variant`` mutrec_def [bir_ieo_condition_def,
                    bir_ieo_variant_def,
                    bir_ieo_invariant_def,
                    bir_ieo_sec_iseven_loop_post_def,
                    bir_ieo_sec_iseven_loop_pre_def,
...
val loop_and_exit_od_ht =
  bir_compose_simp_loop_unsigned_predset
    loop_od_simp_ct loop_od_exit_ht ``bir_ieo_invariant v1`` ``bir_ieo_condition`` ``bir_ieo_variant`` mutrec_def [bir_ieo_condition_def,
                    bir_ieo_variant_def,
                    bir_ieo_invariant_def,
                    bir_ieo_sec_isodd_loop_post_def,
                    bir_ieo_sec_isodd_loop_pre_def,
