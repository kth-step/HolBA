...
* "../1-code/src/add_reg.da": path of the dissassebled program
...
## Transpiler program
The tranpiled program is in the term ``bir_add_reg_prog``. The list of
...
instruction or os generated by some program transformations (i.e. loop
unrolling).
...
```
Notice that due to internals of the ARMv8 semantics the tranpiled
...
program has been correctly tranpiled. 
```
...
overlap. it is aligned, etc.)
  ...
  * the tranpiled program `p` does not contain duplicated labels
* The second part demonstrate the bisimulation (these are
