...
val _ = Datatype `bil_label_t =
    BL_Label string
  | BL_Address bil_imm_t
`;

val _ = Datatype `bil_stmt_t =
  | BStmt_Declare bil_var_t
  | BStmt_Assign  bil_var_t bil_exp_t
  | BStmt_Jmp     bil_label_t
  | BStmt_CJmp    bil_exp_t bil_label_t bil_label_t
  | BStmt_Halt    bil_exp_t
  | BStmt_Assert  bil_exp_t
  | BStmt_Assume  bil_exp_t
`;

val _ = Datatype `bil_block_t = <| bb_label:bil_label_t ; bb_statements:bil_stmt_t list |>`;
val _ = Datatype `bil_program_t = BilProgram (bil_block_t list)`;
val _ = Datatype `bil_programcounter_t = <| bpc_label:bil_label_t; bpc_index:num |>`;

val bil_pc_ss = rewrites (type_rws ``:bil_programcounter_t``);

val _ = Datatype `bil_termcode_t =
    BTC_Running
  | BTC_Halted bil_val_t
  | BTC_ReachedEnd
  | BTC_ReachedUnknownLabel bil_label_t
  | BTC_Failed
  | BTC_AssumptionViolated`

val _ = Datatype `bil_state_t = <|
  bst_pc       : bil_programcounter_t;
  bst_environ  : bil_var_environment_t;
  bst_termcode : bil_termcode_t;
...
val bil_block_stmts_count_def = Define `bil_block_stmts_count bl = LENGTH bl.bb_statements`;
...
val bil_is_valid_labels_def = Define `bil_is_valid_labels p =
  ALL_DISTINCT (bil_labels_of_program p) /\ ~(MEM (BL_Label "") (bil_labels_of_program p))`;
...
REPEAT STRIP_TAC >>
MP_TAC (Q.ISPEC `MAP (\bl. bl.bb_label) (p:bil_block_t list)` listTheory.EL_ALL_DISTINCT_EL_EQ) >>
...
val bil_get_program_block_info_by_label_def = Define `bil_get_program_block_info_by_label
  (BilProgram p) l = INDEX_FIND 0 (\(x:bil_block_t). x.bb_label = l) p
`;

val bil_get_program_block_info_by_label_THM = store_thm ("bil_get_program_block_info_by_label_THM",
  ``(!p l. ((bil_get_program_block_info_by_label (BilProgram p) l = NONE) <=> (!bl. MEM bl p ==> (bl.bb_label <> l)))) /\
...
val bil_is_valid_pc_def = Define `bil_is_valid_pc p pc =
   (?i bl. (bil_get_program_block_info_by_label p (pc.bpc_label) = SOME (i, bl)) /\
           (pc.bpc_index < LENGTH bl.bb_statements))`;

val bil_is_end_pc_def = Define `bil_is_end_pc (BilProgram p) pc =
   (?i bl. (bil_get_program_block_info_by_label (BilProgram p) (pc.bpc_label) = SOME (i, bl)) /\
           (pc.bpc_index = LENGTH bl.bb_statements) /\
...
val bil_is_valid_pc_of_valid_blocks = store_thm ("bil_is_valid_pc_of_valid_blocks",
  ``!p pc. bil_is_valid_labels (BilProgram p) ==>
           (bil_is_valid_pc (BilProgram p) pc <=> (?bl. MEM bl p /\ (pc.bpc_label = bl.bb_label) /\
             (pc.bpc_index < LENGTH bl.bb_statements)))``,
...
val bil_get_program_block_info_by_label_valid_pc = store_thm ("bil_get_program_block_info_by_label_valid_pc",
  ``!p pc. bil_is_valid_pc p pc ==> IS_SOME (bil_get_program_block_info_by_label p pc.bpc_label)``,
...
val bil_get_current_statement_def = Define `bil_get_current_statement p pc =
  option_CASE (bil_get_program_block_info_by_label p pc.bpc_label) NONE
...
REPEAT GEN_TAC >>
Cases_on `bil_get_program_block_info_by_label p pc.bpc_label` >> (
...
val bil_pc_normalise_def = Define `
  (bil_pc_normalise (BilProgram p) pc = case bil_get_program_block_info_by_label (BilProgram p) (pc.bpc_label) of
...
val bil_pc_next_def = Define `
  bil_pc_next p pc = bil_pc_normalise p (pc with bpc_index updated_by SUC)`;


val bil_pc_normalise_EQ_SOME = store_thm ("bil_pc_normalise_EQ_SOME",
``!p pc i bl pc'. (bil_get_program_block_info_by_label (BilProgram p) (pc.bpc_label) = SOME (i, bl)) ==>
...
``!p pc i bl. (bil_get_program_block_info_by_label (BilProgram p) (pc.bpc_label) = SOME (i, bl)) ==>
...
``!p pc. bil_is_valid_pc p pc ==> bil_is_allowed_pc p (pc with bpc_index updated_by SUC)``,
...
``!l p. MEM l (bil_labels_of_program p) ==>
        bil_is_allowed_pc p <| bpc_label := l; bpc_index := 0 |>``,
...
``!p pc pc'. (bil_is_valid_labels p /\ (MEM pc.bpc_label (bil_labels_of_program p)) /\
...
rename1 `bl = EL i p` >>
`bil_get_program_block_info_by_label (BilProgram p) pc.bpc_label = SOME (i, bl)` by
...
val bil_pc_first_def = Define
  `bil_pc_first (BilProgram p) = let bl = HD (FILTER (\bl. (bl.bb_statements) <> []) p) in
    (<| bpc_label := bl.bb_label; bpc_index := 0 |>):bil_programcounter_t`;

val bil_pc_last_def = Define
  `bil_pc_last (BilProgram p) = let bl = LAST (FILTER (\bl. (bl.bb_statements) <> []) p) in
    (<| bpc_label := bl.bb_label; bpc_index := LENGTH bl.bb_statements - 1 |>):bil_programcounter_t`;
...
val bil_is_valid_pc_label_OK = store_thm ("bil_is_valid_pc_label_OK",
  ``!p pc. bil_is_valid_pc p pc ==> MEM pc.bpc_label (bil_labels_of_program p)``,
...
val bil_state_is_terminated_IMP = store_thm ("bil_state_is_terminated_IMP",
  ``(!st. (st.bst_termcode = BTC_Running) ==> ~(bil_state_is_terminated st)) /\
    (!st. (st.bst_termcode <> BTC_Running) ==> (bil_state_is_terminated st))``,
...
val bil_state_normalise_pc_def = Define `bil_state_normalise_pc p (st:bil_state_t) =
  case bil_pc_normalise p st.bst_pc of
...
val bil_state_incr_pc_def = Define `bil_state_incr_pc p (st:bil_state_t) =
  bil_state_normalise_pc p (st with bst_pc := (st.bst_pc with bpc_index updated_by SUC))`

val bil_state_set_pc_def = Define `bil_state_set_pc p (st:bil_state_t) pc' =
  bil_state_normalise_pc p (st with bst_pc := pc')`;

val bil_state_init_def = Define `bil_state_init p = <|
    bst_pc       := bil_pc_first p
  ; bst_environ  := bil_empty_env
...
val bil_declare_initial_value_def = Define `
  (bil_declare_initial_value (BType_Imm _) = NONE) /\
  (bil_declare_initial_value (BType_Mem at vt) = SOME (BVal_Mem at vt (K 0)))`;

val bil_exec_stmt_declare_def = Define `bil_exec_stmt_declare p v ty (st : bil_state_t) =
   let env = (
      let vo = bil_declare_initial_value ty in
      if bil_env_varname_is_bound st.bst_environ v then BEnv_Irregular else
      bil_env_update v vo ty st.bst_environ) in
    if (bil_is_regular_env env) then
       bil_state_incr_pc p (st with bst_environ := env)
    else
...
val bil_exec_stmt_assign_def = Define `bil_exec_stmt_assign p v ex (st : bil_state_t) =
   let env = bil_env_write v (bil_eval_exp ex st.bst_environ) st.bst_environ in
    if (bil_is_regular_env env) then
       bil_state_incr_pc p (st with bst_environ := env)
    else
       bil_state_set_failed st (CONCAT ["irregular environment after "; bil_stmt_to_string
...
val bil_exec_stmt_halt_def = Define `bil_exec_stmt_halt ex (st : bil_state_t) =
    (st with bst_termcode := BTC_Halted (bil_eval_exp ex st.bst_environ))`;

val bil_exec_stmt_jmp_def = Define `bil_exec_stmt_jmp p l (st : bil_state_t) =
    if (MEM l (bil_labels_of_program p)) then
      bil_state_set_pc p st <| bpc_label := l; bpc_index := 0 |>
...
val bil_exec_stmt_cjmp_def = Define `bil_exec_stmt_cjmp p ex l1 l2 (st : bil_state_t) =
  case (bil_dest_bool_val (bil_eval_exp ex st.bst_environ)) of
...
val bil_exec_stmt_def = Define `
  (bil_exec_stmt p (BStmt_Jmp l) st = bil_exec_stmt_jmp p l st) /\
  (bil_exec_stmt p (BStmt_CJmp e l1 l2) st = bil_exec_stmt_cjmp p e l1 l2 st) /\
  (bil_exec_stmt p (BStmt_Declare v) st = bil_exec_stmt_declare p (bil_var_name v) (bil_var_type v) st) /\
  (bil_exec_stmt p (BStmt_Assert ex) st = bil_exec_stmt_assert p ex st) /\
  (bil_exec_stmt p (BStmt_Assume ex) st = bil_exec_stmt_assume p ex st) /\
  (bil_exec_stmt p (BStmt_Assign v ex) st = bil_exec_stmt_assign p v ex st) /\
  (bil_exec_stmt p (BStmt_Halt ex) st = bil_exec_stmt_halt ex st)`;


val bil_exec_step_def = Define `bil_exec_step p state =
  if (bil_state_is_terminated state) then state else
  case (bil_get_current_statement p state.bst_pc) of
    | NONE => bil_state_set_failed state "invalid program_counter"
    | SOME stm => (bil_exec_stmt p stm state) with bst_execs updated_by SUC
`;
...
val bil_is_valid_state_def = Define `bil_is_valid_state p st <=>
  ((bil_is_valid_env st.bst_environ) /\ (if st.bst_termcode = BTC_ReachedEnd then bil_is_end_pc p st.bst_pc else
      bil_is_valid_pc p st.bst_pc))`;
...
SIMP_TAC (std_ss++bil_state_ss++bil_termcode_ss) [bil_is_valid_state_def, bil_state_init_def,
  bil_pc_first_valid, bil_is_valid_env_empty]);


val bil_is_valid_state_exec_ignore = store_thm ("bil_is_valid_state_exec_ignore",
  ``!p st f. bil_is_valid_state p (st with bst_execs updated_by f) =
...
val bil_exec_step_valid_THM = store_thm ("bil_exec_step_valid_THM",
 ``!p st. bil_is_valid_state p st ==>
          (if bil_state_is_terminated st then
             ...
             (bil_is_valid_pc p st.bst_pc) /\
             (?stmt. (bil_get_current_statement p st.bst_pc = SOME stmt) /\
                     (bil_exec_step p st = (bil_exec_stmt p stmt st with bst_execs updated_by SUC))))``,
...
`?stm. bil_get_current_statement p st.bst_pc = SOME stm` by (
  `IS_SOME (bil_get_current_statement p st.bst_pc)` suffices_by METIS_TAC[optionTheory.IS_SOME_EXISTS] >>
...
val bil_is_valid_state_set_failed = store_thm ("bil_is_valid_state_set_failed",
  ``!p st msg. bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc ==>
...
val bil_is_valid_state_incr_pc = store_thm ("bil_is_valid_state_incr_pc",
  ``!p st. bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc ==>
...
Cases_on `bil_pc_normalise p (st.bst_pc with bpc_index updated_by SUC)` >- (
...
val bil_exec_step_valid_state_invar_declare = prove (
  ``!p st v. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
             bil_is_valid_state p (bil_exec_stmt p (BStmt_Declare v) st)``,
Cases_on `v` >> rename1 `BVar v ty` >>
SIMP_TAC (std_ss++boolSimps.LIFT_COND_ss) [bil_exec_stmt_def, bil_exec_stmt_declare_def, LET_THM,
  bil_is_valid_state_set_failed, bil_is_regular_env_def, bil_var_type_def, bil_var_name_def] >>
REPEAT STRIP_TAC >>
...
Q.PAT_X_ASSUM `bil_is_regular_env _` MP_TAC >>
Cases_on `st.bst_environ` >> SIMP_TAC std_ss [bil_env_update_def, bil_is_regular_env_def] >>
COND_CASES_TAC >- SIMP_TAC std_ss [bil_is_regular_env_def] >>
FULL_SIMP_TAC (std_ss) [bil_is_regular_env_def, bil_is_valid_env_def] >>
...
val bil_exec_step_valid_state_invar_assign = prove (
  ``!p st v ex. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
                 bil_is_valid_state p (bil_exec_stmt p (BStmt_Assign v ex) st)``,
Cases_on `v` >> rename1 `BVar v ty` >>
SIMP_TAC (std_ss++boolSimps.LIFT_COND_ss) [bil_exec_stmt_def, bil_exec_stmt_assign_def, LET_THM,
  bil_is_valid_state_set_failed, bil_is_regular_env_def] >>
REPEAT STRIP_TAC >>
MATCH_MP_TAC bil_is_valid_state_incr_pc >>
ASM_SIMP_TAC (std_ss++bil_state_ss) [] >>
FULL_SIMP_TAC (std_ss++bil_state_ss) [bil_is_valid_state_def] >>
...
COND_CASES_TAC >> SIMP_TAC std_ss [bil_is_regular_env_def, bil_var_name_def, bil_var_type_def] >>
Cases_on `st.bst_environ` >> SIMP_TAC std_ss [bil_env_update_def, bil_is_regular_env_def] >>
COND_CASES_TAC >> SIMP_TAC std_ss [bil_is_regular_env_def, bil_is_valid_env_def] >>
ConseqConv.CONSEQ_REWRITE_TAC ([finite_mapTheory.FEVERY_STRENGTHEN_THM], [], []) >>
FULL_SIMP_TAC std_ss [bil_is_valid_env_def]);


val bil_exec_step_valid_state_invar_jmp' = prove (
  ``!p st l. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
...
COND_CASES_TAC >| [
  ...
  Cases_on `bil_pc_normalise p <|bpc_label := l; bpc_index := 0|>` >| [
    FULL_SIMP_TAC (std_ss++bil_state_ss) [bil_is_valid_state_def] >>
...
val bil_exec_step_valid_state_invar_jmp = prove (
  ``!p st l. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
             bil_is_valid_state p (bil_exec_stmt p (BStmt_Jmp l) st)``,
SIMP_TAC std_ss [bil_exec_stmt_def, bil_exec_step_valid_state_invar_jmp']);


val bil_exec_step_valid_state_invar_cjmp = prove (
  ``!p st ex l1 l2.
       (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
       bil_is_valid_state p (bil_exec_stmt p (BStmt_CJmp ex l1 l2) st)``,
SIMP_TAC std_ss [bil_exec_stmt_def, bil_exec_stmt_cjmp_def] >>
REPEAT STRIP_TAC >>
Cases_on `bil_dest_bool_val (bil_eval_exp ex st.bst_environ)` >- (
...
val bil_exec_step_valid_state_invar_halt = prove (
  ``!p st ex. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
               bil_is_valid_state p (bil_exec_stmt p (BStmt_Halt ex) st)``,
...
val bil_exec_step_valid_state_invar_assert = prove (
  ``!p st ex. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
               bil_is_valid_state p (bil_exec_stmt p (BStmt_Assert ex) st)``,
...
val bil_exec_step_valid_state_invar_assume = prove (
  ``!p st ex. (bil_is_valid_program p /\ bil_is_valid_state p st /\ bil_is_valid_pc p st.bst_pc) ==>
               bil_is_valid_state p (bil_exec_stmt p (BStmt_Assume ex) st)``,
...
val bil_state_normalise_pc_execs = store_thm ("bil_state_normalise_pc_execs",
  ``!p st. (bil_state_normalise_pc p st).bst_execs = st.bst_execs``,
REPEAT GEN_TAC >>
SIMP_TAC std_ss [bil_state_normalise_pc_def] >>
Cases_on `bil_pc_normalise p st.bst_pc` >> (
  ASM_SIMP_TAC (std_ss++bil_state_ss) []
));


val bil_state_incr_pc_execs = store_thm ("bil_state_incr_pc_execs",
  ``!p st. (bil_state_incr_pc p st).bst_execs = st.bst_execs``,
...
val bil_state_set_failed_execs = store_thm ("bil_state_set_failed_execs",
  ``!st msg. (bil_state_set_failed st msg).bst_execs = st.bst_execs``,
SIMP_TAC (std_ss++bil_state_ss) [bil_state_set_failed_def]);


val bil_state_set_pc_execs = store_thm ("bil_state_set_pc_execs",
  ``!st p pc'. (bil_state_set_pc p st pc').bst_execs = st.bst_execs``,
...
``!p st. bil_is_valid_state p st ==>
         (if bil_state_is_terminated st then
           (bil_exec_step p st = st)
         else ((bil_exec_step p st).bst_execs = SUC (st.bst_execs)))``,
...
Cases >> ASM_SIMP_TAC std_ss [bil_exec_stmt_def] >| [
  ...
  Q.HO_MATCH_ABBREV_TAC `bil_state_EQ_EXCEPT_EXECS (if (bil_is_regular_env env) then _ else _) _` >>
  CASE_TAC >- (
...
Cases_on `bil_get_current_statement p st1.bst_pc` >- (
  ASM_SIMP_TAC (std_ss++bil_state_ss) [bil_state_EQ_EXCEPT_EXECS_ALT_DEF,
...
``!p state n.
    ...
    ((FUNPOW (bil_exec_step p) n state).bst_execs = state.bst_execs + n)``,
...
val bil_state_set_failed_SAME_ENV = store_thm ("bil_state_set_failed_SAME_ENV",
  ``!st msg. (bil_state_set_failed st msg).bst_environ = st.bst_environ``,
SIMP_TAC (std_ss++bil_state_ss) [bil_state_set_failed_def]);


val bil_state_normalise_pc_SAME_ENV = store_thm ("bil_state_normalise_pc_SAME_ENV",
  ``!p st. (bil_state_normalise_pc p st).bst_environ = st.bst_environ``,
REPEAT STRIP_TAC >>
SIMP_TAC std_ss [bil_state_normalise_pc_def] >>
...
val bil_state_incr_pc_SAME_ENV = store_thm ("bil_state_incr_pc_SAME_ENV",
  ``!p st. (bil_state_incr_pc p st).bst_environ = st.bst_environ``,
...
val bil_state_set_pc_SAME_ENV = store_thm ("bil_state_set_pc_SAME_ENV",
  ``!p st l. (bil_state_set_pc p st l).bst_environ = st.bst_environ``,
...
val bil_exec_stmt_jmp_SAME_ENV = store_thm("bil_exec_stmt_jmp_SAME_ENV",
  ``!p st l. (bil_exec_stmt_jmp p l st).bst_environ = st.bst_environ``,
SIMP_TAC std_ss [bil_exec_stmt_jmp_def] >>
REPEAT STRIP_TAC >>
...
val bil_exec_stmt_cjmp_SAME_ENV = store_thm("bil_exec_stmt_cjmp_SAME_ENV",
  ``!p e st l1 l2. (bil_exec_stmt_cjmp p e l1 l2 st).bst_environ = st.bst_environ``,
...
val bil_exec_stmt_halt_SAME_ENV = store_thm("bil_exec_stmt_halt_SAME_ENV",
  ``!e st. (bil_exec_stmt_halt e st).bst_environ = st.bst_environ``,
...
val bil_exec_stmt_assert_SAME_ENV = store_thm("bil_exec_stmt_assert_SAME_ENV",
  ``!p e st. (bil_exec_stmt_assert p e st).bst_environ = st.bst_environ``,
...
val bil_exec_stmt_assume_SAME_ENV = store_thm("bil_exec_stmt_assume_SAME_ENV",
  ``!p e st. (bil_exec_stmt_assume p e st).bst_environ = st.bst_environ``,
...
val bil_exec_stmt_declare_ENV = store_thm("bil_exec_stmt_declare_ENV",
  ``!p vn vty st. (bil_exec_stmt_declare p vn vty st).bst_environ =
      if (bil_env_varname_is_bound st.bst_environ vn) then st.bst_environ else
...
REPEAT CASE_TAC >> (
   FULL_SIMP_TAC (std_ss++bil_state_ss) [bil_state_incr_pc_SAME_ENV,
     bil_state_set_failed_SAME_ENV, bil_is_regular_env_def]
) >>
Cases_on `st.bst_environ` >> FULL_SIMP_TAC std_ss [bil_env_update_def] >>
Cases_on `vty` >> FULL_SIMP_TAC std_ss [bil_declare_initial_value_def,
  bil_is_regular_env_def, type_of_bil_val_def]);


val bil_exec_stmt_assign_ENV = store_thm("bil_exec_stmt_assign_ENV",
  ``!p v e st.
      (bil_exec_stmt_assign p v e st).bst_environ =
      (if bil_is_regular_env (bil_env_write v (bil_eval_exp e st.bst_environ) st.bst_environ) then
         (bil_env_write v (bil_eval_exp e st.bst_environ) st.bst_environ)
       else st.bst_environ)``,
...
``!p st stmt. bil_env_order st.bst_environ (bil_exec_stmt p stmt st).bst_environ``,
...
Cases_on `stmt` >> (
  ...
  Cases_on `bil_env_varname_is_bound st.bst_environ (bil_var_name v)` >> ASM_REWRITE_TAC[] >>
...
``!p st. bil_env_order st.bst_environ (bil_exec_step p st).bst_environ``,
...
``!p n st. bil_env_order st.bst_environ (FUNPOW (bil_exec_step p) n st).bst_environ``,
...
Q.EXISTS_TAC `(bil_exec_step p st).bst_environ` >>
...
``!p c st st'. (bil_exec_steps p st = SOME (c, st')) ==>
  bil_env_order st.bst_environ st'.bst_environ``,
