...
val bir_is_well_typed_stmt_def = Define `
  (bir_is_well_typed_stmt (BStmt_Declare _) = T) /\
  (bir_is_well_typed_stmt (BStmt_Assign v e) = (type_of_bir_exp e = SOME (bir_var_type v))) /\
  (bir_is_well_typed_stmt (BStmt_Jmp _) = T) /\
  (bir_is_well_typed_stmt (BStmt_CJmp c _ _) = (type_of_bir_exp c = SOME BoolType)) /\
  (bir_is_well_typed_stmt (BStmt_Halt e) = (type_of_bir_exp e <> NONE)) /\
  (bir_is_well_typed_stmt (BStmt_Assert e) = (type_of_bir_exp e = SOME BoolType)) /\
  (bir_is_well_typed_stmt (BStmt_Assume e) = (type_of_bir_exp e = SOME BoolType))`


val bir_is_well_typed_program_def = Define `bir_is_well_typed_program (BirProgram p) <=>
  (EVERY (\bl. EVERY bir_is_well_typed_stmt bl.bb_statements) p)`;
