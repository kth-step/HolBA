...
open bil_auxiliaryTheory;

val _ = new_theory "bil_imm";

(* ------------------------------------------------------------------------- *)
(*  Immediates                                                               *)
(* ------------------------------------------------------------------------- *)

(* Bil immediates can be words of various sizes *)
val _ = Datatype `bil_imm_t =
    Imm1  bool[1]
  | Imm8  bool[8]
  | Imm16 bool[16]
  | Imm32 bool[32]
...
val type_of_bil_imm_def = Define `
  (type_of_bil_imm (Imm1  _) = Bit1) /\
  (type_of_bil_imm (Imm8  _) = Bit8) /\
  (type_of_bil_imm (Imm16 _) = Bit16) /\
  (type_of_bil_imm (Imm32 _) = Bit32) /\
  (type_of_bil_imm (Imm64 _) = Bit64)`;

val size_of_bil_immtype_def = Define `
  (size_of_bil_immtype Bit1  = 1) /\
  (size_of_bil_immtype Bit8  = 8) /\
  (size_of_bil_immtype Bit16 = 16) /\
  (size_of_bil_immtype Bit32 = 32) /\
  (size_of_bil_immtype Bit64 = 64)`

val bil_immtype_of_size_def = Define `
  bil_immtype_of_size n = (
...
Cases >> Cases >> SIMP_TAC (std_ss++bil_imm_ss) [size_of_bil_immtype_def]);

val size_of_bil_immtype_NEQ_0 = store_thm ("size_of_bil_immtype_NEQ_0",
  ``!b. size_of_bil_immtype b <> 0``,
Cases >> SIMP_TAC arith_ss [size_of_bil_immtype_def])
...
SIMP_TAC std_ss [is_valid_bil_immtype_size_REWRS, bil_immtype_of_size_def] >>
METIS_TAC[optionTheory.IS_SOME_DEF]);

val bil_immtype_of_num_inv = store_thm ("bil_immtype_of_num_inv",
  ``(!brt. bil_immtype_of_size (size_of_bil_immtype brt) = SOME brt) /\
    (!n brt. (bil_immtype_of_size n = SOME brt) ==>
             (size_of_bil_immtype brt = n))``,
REPEAT CONJ_TAC >| [
  Cases >> SIMP_TAC std_ss [size_of_bil_immtype_def, bil_immtype_of_size_def],

  SIMP_TAC (std_ss++boolSimps.LIFT_COND_ss) [bil_immtype_of_size_def, DISJ_IMP_THM, FORALL_AND_THM,
    COND_EXPAND_OR, LEFT_AND_OVER_OR, size_of_bil_immtype_def]
]);
...
val b2n_n2bs = store_thm ("b2n_n2bs",
  ``!bt n. b2n (n2bs n bt) = MOD_2EXP (size_of_bil_immtype bt) n``,
Cases >> (
  SIMP_TAC (std_ss++wordsLib.WORD_ss) [b2n_def, n2bs_def, w2n_n2w,
    size_of_bil_immtype_def, bitTheory.MOD_2EXP_def]
));
...
val b2n_n2b_fixed = store_thm ("b2n_n2b_fixed",
  ``(!n. b2n (n2b_1 n)  = n MOD 2) /\
    ...
    (!n bt. b2n (n2bs n bt) = n MOD 2**(size_of_bil_immtype bt))``,

SIMP_TAC std_ss [n2b_fixed_DEFS, b2n_n2bs, size_of_bil_immtype_def,
  bitTheory.MOD_2EXP_def]);

val n2bs_b2n = store_thm ("n2bs_b2n",
  ``!b s. (s = type_of_bil_imm b) ==> (n2bs (b2n b) s = b)``,
Cases >> (
  SIMP_TAC std_ss [b2n_def, n2bs_def, type_of_bil_imm_def, n2w_w2n]
));
...
val b2n_lt = store_thm ("b2n_lt",
  ``!b. b2n b < 2 ** (size_of_bil_immtype (type_of_bil_imm b))``,
Cases >> (
  SIMP_TAC std_ss [b2n_def, type_of_bil_imm_def, size_of_bil_immtype_def] >>
...
SIMP_RULE (std_ss++DatatypeSimps.expand_type_quants_ss[``:bil_imm_t``]) [
  type_of_bil_imm_def, size_of_bil_immtype_def] b2n_lt);


val b2n_MOD_2EXP = store_thm ("b2n_MOD_2EXP",
  ``!i n. ((size_of_bil_immtype (type_of_bil_imm i)) = n) ==>
          (MOD_2EXP n (b2n i) = b2n i)``,
SIMP_TAC std_ss [bitTheory.MOD_2EXP_def, b2n_lt]);


val type_of_n2bs = store_thm ("type_of_n2bs",
``!s n. type_of_bil_imm (n2bs n s) = s``,
Cases >> SIMP_TAC std_ss [n2bs_def, type_of_bil_imm_def]);
...
val type_of_bil_imm_n2bs_INTRO = store_thm ("type_of_bil_imm_n2bs_INTRO",
  ``!s b. (type_of_bil_imm b = s) <=> (?n. b = n2bs n s)``,
...
val w2bs_b2w = store_thm ("w2bs_b2w",
  ``!b s. ((s = type_of_bil_imm b) /\ size_of_bil_immtype (type_of_bil_imm b) <= dimindex (:'a)) ==>
...
`b2n b MOD (dimword (:'a)) = b2n b` by (
  ...
  `2 ** size_of_bil_immtype (type_of_bil_imm b) <= 2 ** dimindex (:'a)` by
...
``!s w. type_of_bil_imm (w2bs w s) = s``,
...
val type_of_bool2b = store_thm ("type_of_bool2b",
  ``!b. type_of_bil_imm (bool2b b) = Bit1``,
Cases >> SIMP_TAC std_ss [bool2b_def, type_of_bil_imm_def]);
...
val b2v_LENGTH = store_thm ("b2v_LENGTH",
  ``!r. LENGTH (b2v r) = size_of_bil_immtype (
        type_of_bil_imm r)``,
Cases >> (
  SIMP_TAC (std_ss++wordsLib.WORD_ss) [b2v_def, type_of_bil_imm_def,
    size_of_bil_immtype_def, length_w2v]
));
...
``!s v. type_of_bil_imm (v2bs v s) = s``,
