...
Datatype:
  bir_machine_lifted_imm_t = BMLI bir_var_t ('ms -> bir_imm_t)
End

Definition bir_machine_lifted_imm_OK_def:
  bir_machine_lifted_imm_OK (BMLI v (lf : 'ms -> bir_imm_t)) <=>
(~(bir_is_temp_var_name (bir_var_name v))) /\
(!ms. BType_Imm (type_of_bir_imm (lf ms)) = (bir_var_type v))
End


Definition bir_machine_lifted_imm_def:
...
Theorem bir_machine_lifted_imm_OK_IMPLIES_NO_TEMP_VAR:
...
Theorem bir_machine_lifted_imm_INITIALISED:
...
Theorem bir_machine_lifted_imm_DECLARED:
  !v lf bs ms.
      bir_machine_lifted_imm (BMLI v lf) bs ms ==>
...
Theorem bir_machine_lifted_imm_DECLARED_TEMP:
...
Theorem bir_machine_lifted_imm_LIFTED:
  !v lf bs ms. bir_machine_lifted_imm_OK (BMLI v lf) ==>
                 bir_machine_lifted_imm (BMLI v lf) bs ms ==>
...
Definition bir_machine_lifted_imms_def:
  bir_machine_lifted_imms vl bs ms =
...
Datatype:
  bir_machine_lifted_mem_t = BMLM bir_var_t ('ms -> 'a word -> 'b word)
End

Definition bir_machine_lifted_mem_OK_def:
  bir_machine_lifted_mem_OK (BMLM v (lf : 'ms -> 'a word -> 'b word)) <=>
  (?sa sb.
...
Definition bir_machine_lifted_mem_def:
...
Theorem bir_machine_lifted_mem_OK_IMPLIES_NO_TEMP_VAR:
...
Theorem bir_machine_lifted_mem_INITIALISED:
...
Theorem bir_machine_lifted_mem_DECLARED:
  !v lf bs ms.
      bir_machine_lifted_mem (BMLM v lf) bs ms ==>
      (bir_env_var_is_declared bs.bst_environ v)
Proof
...
Theorem bir_machine_lifted_mem_DECLARED_TEMP:
  !v ty lf bs ms use_temp.
       bir_machine_lifted_mem (BMLM v lf) bs ms ==>
       (bir_env_var_is_declared bs.bst_environ (bir_temp_var use_temp v))
Proof
...
Theorem bir_machine_lifted_mem_LIFTED:
  !v lf bs ms. bir_machine_lifted_mem_OK (BMLM v lf) ==>
                 bir_machine_lifted_mem (BMLM v lf) bs ms ==>
...
Datatype:
  bir_machine_lifted_pc_t = BMLPC bir_var_t bir_var_t ('ms -> bir_imm_t)
End

Definition bir_machine_lifted_pc_OK_def:
  bir_machine_lifted_pc_OK (BMLPC v_pc v_pc_cond (lf : 'ms -> bir_imm_t)) <=>
...
Definition bir_machine_lifted_pc_def:
...
Theorem bir_machine_lifted_pc_DECLARED:
...
Definition bmr_mem_lf_def:
  bmr_mem_lf r = case r.bmr_mem of BMLM v lf => lf
End
Definition bmr_mem_var_def:
  bmr_mem_var r = case r.bmr_mem of BMLM v lf => v
End
Definition bmr_pc_lf_def:
  bmr_pc_lf r = case r.bmr_pc of BMLPC v_pc v_pc_cond lf => lf
End
Definition bmr_pc_var_def:
  bmr_pc_var r = case r.bmr_pc of BMLPC v_pc v_pc_cond lf => v_pc
End
Definition bmr_pc_var_cond_def:
  bmr_pc_var_cond r = case r.bmr_pc of BMLPC v_pc v_pc_cond lf => v_pc_cond
End

Definition bmr_mem_addr_sz_def:
  bmr_mem_addr_sz r =
  (case (bir_var_type (bmr_mem_var r)) of
      (BType_Mem addr_sz _) => addr_sz)
End

Definition bmr_mem_val_sz_def:
  bmr_mem_val_sz r =
  (case (bir_var_type (bmr_mem_var r)) of
      (BType_Mem _ val_sz) => val_sz)
End

Definition bmr_vars_def:
  bmr_vars r =
  (case r.bmr_mem of (BMLM v _) => v)::
  (MAP (\i. case i of (BMLI v _) => v) r.bmr_imms)
End

Definition bmr_temp_vars_def:
  bmr_temp_vars r =
  (case r.bmr_pc of BMLPC v1 v2 _ => [v1;v2]) ++
  (MAP (bir_temp_var T) (bmr_vars r))
End

Definition bmr_varnames_distinct_def:
  bmr_varnames_distinct r <=>
  ALL_DISTINCT (MAP bir_var_name (bmr_vars r ++ bmr_temp_vars r))
End

Definition bmr_ok_def:
  bmr_ok r <=>
...
Theorem bmr_temp_vars_TEMP_VAR:
  !r. bmr_ok r ==>
...
Theorem bmr_temp_vars_DECLARED:
  !r bs ms. bmr_rel r bs ms ==>
...
Theorem bmr_rel_IMPL_IS_BL_Block_Address:
  !r bs ms. bmr_rel r bs ms ==>
    bir_state_COUNT_PC
...
Theorem bmr_rel_RECOVER_FROM_JUMP_OUTSIDE:
  !r bs ms l.
      bmr_rel r bs ms ==>
...
Definition bmr_ms_mem_contains_def:
...
Definition bmr_ms_mem_contains_interval_def:
  bmr_ms_mem_contains_interval (ba: 'a word, wl:'b word list) <=>
  (WI_size ba (n2w (LENGTH wl)))
End

Definition WF_bmr_ms_mem_contains_def:
  WF_bmr_ms_mem_contains (ba: 'a word, wl:'b word list) <=>
  (LENGTH wl < dimword (:'a)) /\
  (WI_wf (bmr_ms_mem_contains_interval (ba, wl)))
End

Theorem WF_bmr_ms_mem_contains_ALT_DEF:
...
Definition arm8_REGS_lifted_imms_LIST_def:
...
(* Lifting PSTATE *)
Definition arm8_PSTATE_lifted_imms_LIST_def:
...
Definition arm8_EXTRA_lifted_imms_LIST_def:
...
Theorem bmr_ms_mem_contains_ARM8:
  !ms v1 v2 v3 v4.
...
Definition m0_REGS_lifted_imms_LIST_def:
...
Theorem bmr_ms_mem_contains_M0_4:
...
Theorem bmr_ms_mem_contains_M0_2:
  !ef sel ms v1 v2.
...
Definition riscv_state_is_OK_def:
 ...
 * ra, and so on. *)
Definition riscv_GPRS_lifted_imms_LIST_def:
 ...
 * a process ID. *)
Definition riscv_lifted_mem_def:
...
Definition riscv_lifted_pc_def:
...
Theorem bmr_ms_mem_contains_RISCV:
  !ms v1 v2 v3 v4.
...
Definition m0_mod_REGS_lifted_imms_LIST_def:
...
Theorem bmr_ms_mem_contains_M0_mod_4:
  !ef sel ms v1 v2 v3 v4.
...
Theorem bmr_ms_mem_contains_M0_mod_2:
  !ef sel ms v1 v2.
