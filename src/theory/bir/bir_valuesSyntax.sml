...
val (BVal_Known_tm,  mk_BVal_Known, dest_BVal_Known, is_BVal_Known)  = syntax_fns1 "BVal_Known";
val (BKVal_Imm_tm,  mk_BKVal_Imm, dest_BKVal_Imm, is_BKVal_Imm)  = syntax_fns1 "BKVal_Imm";
val (BKVal_Mem_tm,  mk_BKVal_Mem, dest_BKVal_Mem, is_BKVal_Mem)  = syntax_fns3 "BKVal_Mem";

val dest_BKVal_Imm1  = dest_Imm1 o dest_BKVal_Imm
val dest_BKVal_Imm8  = dest_Imm8 o dest_BKVal_Imm
val dest_BKVal_Imm16 = dest_Imm16 o dest_BKVal_Imm
val dest_BKVal_Imm32 = dest_Imm32 o dest_BKVal_Imm
val dest_BKVal_Imm64 = dest_Imm64 o dest_BKVal_Imm
val gen_dest_BKVal_Imm = gen_dest_Imm o dest_BKVal_Imm

val is_BKVal_Imm1  = can dest_BKVal_Imm1;
val is_BKVal_Imm8  = can dest_BKVal_Imm8;
val is_BKVal_Imm16 = can dest_BKVal_Imm16;
val is_BKVal_Imm32 = can dest_BKVal_Imm32;
val is_BKVal_Imm64 = can dest_BKVal_Imm64;
fun gen_is_BKVal_Imm n t = (fst (gen_dest_BKVal_Imm t) = n) handle HOL_ERR _ => false;


fun mk_BKVal_MemByte (aty, mf) = mk_BKVal_Mem (aty, Bit8_tm, mf);

fun dest_BKVal_MemByte tm = let
  val (aty, vty, mf) = dest_BKVal_Mem tm;
  val _ = if is_Bit8 vty then () else fail();
in (aty, mf) end handle e => raise wrap_exn "dest_BKVal_MemByte" e;

val is_BKVal_MemByte = can dest_BKVal_MemByte;

fun mk_BKVal_MemByte_32 mf = mk_BKVal_Mem (Bit32_tm, Bit8_tm, mf);

fun dest_BKVal_MemByte_32 tm = let
  val (aty, mf) = dest_BKVal_MemByte tm;
  val _ = if is_Bit32 aty then () else fail();
in mf end handle e => raise wrap_exn "dest_BKVal_MemByte_32" e;

val is_BKVal_MemByte_32 = can dest_BKVal_MemByte_32;

fun mk_BKVal_MemByte_64 mf = mk_BKVal_Mem (Bit64_tm, Bit8_tm, mf);
fun dest_BKVal_MemByte_64 tm = let
  val (aty, mf) = dest_BKVal_MemByte tm;
  val _ = if is_Bit64 aty then () else fail();
in mf end handle e => raise wrap_exn "dest_BKVal_MemByte_32" e;

val is_BKVal_MemByte_64 = can dest_BKVal_MemByte_64;
