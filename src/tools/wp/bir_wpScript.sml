...
``,
 ...
 Q.ABBREV_TAC `s' = bir_exec_to_labels ls p s` >>
(*
 Q.ABBREV_TAC `cnd1 = (l1 ∈ ls ⇒ (post1' = post)) ∧
           (l1 ∉ ls ⇒
            bir_exec_to_labels_triple p l1 ls post1' post ∧
            bir_is_bool_exp post1')` >>
 Q.ABBREV_TAC `cnd2 = (l2 ∈ ls ⇒ (post2' = post)) ∧
            (l2 ∉ ls ⇒
...
*)
 ...
 (
  Cases_on `l1 IN ls` >> (
    FULL_SIMP_TAC std_ss [](*Abbr `cnd1`]*)
  )
 ) >> FULL_SIMP_TAC std_ss [] >>
 subgoal `bir_is_bool_exp post2'` >- 
 (
  Cases_on `l2 IN ls` >> (
    FULL_SIMP_TAC std_ss [](*Abbr `cnd2`]*)
  )
 ) >> FULL_SIMP_TAC std_ss [] >>

(*
 FULL_SIMP_TAC std_ss [Abbr `cnd1`,Abbr `cnd1`] >>
*)
 REV_FULL_SIMP_TAC std_ss [] >>
...
Cases_on `l1 IN  ls` >-
 (
   FULL_SIMP_TAC std_ss [(*Abbr `cnd1`],*) LET_DEF, bir_state_COUNT_PC_def] >>
   ASSUME_TAC (Q.SPECL [`(BirProgram l')`, `bl`, `s`, `st10`, `st11`,`st12`] bir_exec_block_runing_at_least_one_step) >>
   REV_FULL_SIMP_TAC std_ss [] >>
   FULL_SIMP_TAC std_ss [] >>
   FULL_SIMP_TAC (srw_ss()) [bir_block_pc_def, Abbr `s'`]
  ...
  | NONE    => (
      let bl = SND(THE(bir_get_program_block_info_by_label p l)) in
        case bl.bb_last_statement of
	      ...
	      NONE => NONE
            | SOME wp => SOME (wps |+ (l, (bir_wp_exec_stmtsB bl.bb_statements wp)))
            )
...
``
!p.
    (bir_jmp_direct_labels_only p = bir_jmp_direct_labels_only_exec p)
``,
  ...
  GEN_TAC >> EQ_TAC >- (
    REPEAT STRIP_TAC >- (
      Q.PAT_X_ASSUM `!x.y` (fn thm => ASSUME_TAC (Q.SPEC `h` thm)) >>
      FULL_SIMP_TAC (srw_ss()) []
    ) >>
...
!p l1.
    ...
    (bir_edges_blocks_in_prog p l1 = bir_edges_blocks_in_prog_exec p l1)
``,
  ...
  REWRITE_TAC [bir_targets_in_prog_exec_def, bir_edge_in_prog_def, bir_edges_blocks_in_prog_def, bir_edges_blocks_in_prog_exec_def, listTheory.EVERY_MEM, listTheory.EXISTS_MEM, LET_DEF] >>
  BETA_TAC >>
  ...
  EQ_TAC >- (
    REPEAT STRIP_TAC >> Cases_on `bl1.bb_last_statement` >|
    [
      Cases_on `b`
    ,
      Cases_on `b0` >> (Cases_on `b1`)
    ,
      ...
      FULL_SIMP_TAC (srw_ss()) [] >> METIS_TAC [rich_listTheory.EL_MEM]
    )
  ) >>

  REPEAT STRIP_TAC >> (
    subgoal `bl1' = bl1` >- (
      FULL_SIMP_TAC (std_ss) [bir_program_valid_stateTheory.bir_is_valid_labels_def, bir_labels_of_program_def, listTheory.MEM_EL] >>
...
val FEVERY_FEVERY_DRESTRICT_thm = prove(``
    !P M S. FEVERY P M ==> FEVERY P (DRESTRICT M S)
``,

  FULL_SIMP_TAC (std_ss++pred_setSimps.PRED_SET_ss) [finite_mapTheory.FEVERY_DEF,
  ...
  Cases_on `FLOOKUP wps l` >- (
    ...
    [
      Cases_on `l1 IN FDOM wps` >- (
        ...
        Q.ABBREV_TAC `exp1 = bir_wp_exec_stmtsB bl.bb_statements (wps ' l1)` >>
    ...
    ,
      ...
      Cases_on `l1 IN FDOM wps /\ l2 IN FDOM wps` >- (
        ...
        subgoal `bir_is_bool_exp (exp1)` >- (
          subgoal `bir_is_well_typed_stmtE (BStmt_CJmp e (BLE_Label l1) (BLE_Label l2))` >- (
            METIS_TAC [bir_is_well_typed_program_def, listTheory.EVERY_MEM, bir_is_well_typed_block_def]
          ) >>

          FULL_SIMP_TAC (std_ss) [bir_is_bool_exp_REWRS, finite_mapTheory.FEVERY_DEF, bir_is_well_typed_stmtE_def, bir_is_bool_exp_GSYM, Abbr `exp1`]
        ) >>

        subgoal `bir_is_bool_exp (bir_wp_exec_stmtsB bl.bb_statements exp1)` >- (
          FULL_SIMP_TAC (std_ss) [bir_declare_free_prog_def] >>
          METIS_TAC [bir_is_well_typed_program_def, bir_is_well_typed_block_def, listTheory.EVERY_MEM, bir_wp_exec_stmtsB_bool_thm]
        ) >>
...
reworked area follows here

*)


val bir_bool_wps_map_def = Define `
      bir_bool_wps_map wps = (FEVERY (\(l1, wp1). bir_is_bool_exp wp1) wps)
`;

val bir_sound_wps_map_def = Define `
      bir_sound_wps_map p ls post wps = (FEVERY (\(l1, wp1). ((l1 IN ls) ==> (wp1 = post)) /\
                         ((~(l1 IN ls)) ==> (bir_exec_to_labels_triple p l1 ls wp1 post))
            ) wps)
`;
...
``,
  ...
  METIS_TAC [bir_declare_free_prog_exec_eq_thm, bir_wp_exec_of_block_bool_thm]
);

val bir_wp_exec_of_block_sound_exec_thm = store_thm("bir_wp_exec_of_block_sound_exec_thm",
``
!p l ls post wps wps'.
    (bir_is_bool_exp post) ==>
    (bir_is_well_typed_program p) ==>
...
!p l ls post wps wps'.
    ...
    (bir_edges_blocks_in_prog_exec p l) ==>
    (~(l IN ls)) ==>
    ((bir_bool_wps_map wps) /\ (bir_sound_wps_map p ls post wps)) ==>
    ((bir_wp_exec_of_block p l ls post wps) = SOME wps') ==>
    ((bir_bool_wps_map wps') /\ (bir_sound_wps_map p ls post wps'))
``,
