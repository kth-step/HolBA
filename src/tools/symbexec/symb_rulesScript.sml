...
(* TODO: split this into two *)
val symb_FRESH_SIMP_matchstate_IMP_matchstate_ext_thm = store_thm(
   "symb_FRESH_SIMP_matchstate_IMP_matchstate_ext_thm", ``
!sr.
(symb_symbols_f_sound sr) ==>
(symb_typeof_exp_sound sr) ==>
(symb_val_eq_sound sr) ==>
...
(!var symb_exp symb symbexp symbexp' sys sys' H s.
...
``,
  ...
  Q.ABBREV_TAC `H1 = symb_interpr_update H (symb, sr.sr_interpret_f H symb_exp)` >>
  Q.EXISTS_TAC `H1` >>
  ...
  `symb_interpr_welltyped sr H1` by (
    `symb_interpr_welltyped sr H` by METIS_TAC [symb_matchstate_def] >>
    `sr.sr_symbols_f symb_exp SUBSET symb_interpr_dom H` by (
      cheat
      (*METIS_TAC
        [symb_matchstate_def, symb_suitable_interpretation_def,
         symb_interpr_for_symbs_def, symb_symbols_SUBSET_store_exps_thm2, SUBSET_TRANS]*)
  ...
  `?var_v. sr.sr_interpret_f H symb_exp = SOME var_v` by (
    cheat
    (* METIS_TAC [symb_matchstate_IMP_interpret_exp_SOME_thm] *)
  ) >>
  `sr.sr_interpret_f H1 symb_exp = SOME var_v` by (
    cheat
    (*METIS_TAC [symb_matchstate_ext_IMP_SAME_interpret_exp_thm]*)
  ) >>
  FULL_SIMP_TAC std_ss [] >>
  ...
  (* now the prerequisites for the store update *)
  `sr.sr_symbols_f symb_exp SUBSET symb_interpr_dom H1` by (
    cheat (*
    METIS_TAC
      [symb_matchstate_def, symb_suitable_interpretation_def, symb_interpr_for_symbs_def,
       symb_symbols_SUBSET_store_exps_thm2, SUBSET_TRANS] *)
...
!var symb symb_exp symbexp symbexp' sys sys' H H' s.
  ...
  (sr.sr_symbols_f symb_exp SUBSET symb_interpr_dom H) ==>
...
``,
  ...
  `symb_interpr_for_symbs
              (sr.sr_symbols_f (symb_expr_conj_eq sr (sr.sr_mk_exp_symb_f symb) symb_exp (symb_symbst_pcond sys)) UNION
               sr.sr_symbols_f symbexp UNION sr.sr_symbols_f symbexp') H'` by (
    cheat (*
    METIS_TAC
      [symb_interpr_for_symbs_def, UNION_SUBSET, SUBSET_TRANS,
  ...
  `sr.sr_interpret_f H' (symb_expr_conj_eq sr (sr.sr_mk_exp_symb_f symb) symb_exp (symb_symbst_pcond sys)) = SOME sr.sr_val_true` by (
    cheat
  ) >>
...
``,
  ...
  (* from H', make an H_m minimal wrt sys2, added with sys mappings *)
  (* this is an extension from H *)
  `symb_interpr_ext H_e H` by (
(*
    METIS_TAC [symb_interpr_extend_symbs_IMP_ext_thm, symb_interpr_ext_TRANS_thm,
...
*)
  ...
  `symb_matchstate sr sys2 H_e s'` by (
    cheat
  ) >>
...
(*  rename1 `symb_matchstate sr sys' H_m s'` >> *)
  ...
  (* this is also an extension from H *)
  `symb_interpr_ext H_m H` by (
(*
    METIS_TAC [symb_interpr_extend_symbs_IMP_ext_thm, symb_interpr_ext_TRANS_thm,
      symb_interpr_extend_symbs_sr_def]
*)
...
*)
  ...
  `sr.sr_symbols_f symb_exp SUBSET symb_interpr_dom H_e` by (
    cheat (* TODO: need to add proper assumption *)
  ) >>
...
(!sys L Pi sys2 sys2' var symb_exp symb symbexp symbexp'.
  ...
  (sr.sr_symbols_f symb_exp SUBSET symb_symbols sr sys2) ==>
