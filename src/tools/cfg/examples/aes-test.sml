...
val blocks = List.tabulate (7, fn x => ``<|bb_label := BL_Address (Imm32 ^(wordsSyntax.mk_word(Arbnum.fromInt x, Arbnum.fromInt 32)));
   bb_statements :=
     [];
   bb_last_statement :=
     BStmt_Jmp (BLE_Label (BL_Address (Imm32 33030w)))|>``);
val in_idxs  = [[],[0],[0],[1,2,6],[3],[4],[4]];
val out_idxs = [[1,2],[3],[3],[4],[5,6],[~1],[3]];

is_reachable [4,3] [5,4] 2

*)

val _ = if printgraphs then bir_show_graph_inout_all true (blocks,in_idxs,out_idxs) else ();
...
val _ = if printgraphs then
          (List.map (fn (nodes,_,_) => bir_show_graph_inout true nodes (blocks, in_idxs, out_idxs)) lf_frags; ())
        else ();
